...
class RacingGame:
    ...
    def update_track(self) -> None:
        ...
        new_offset = max(1, min(COLS - TRACK_WIDTH - 10, new_offset))
    ...
    def update(self, now: float) -> None:
        while self.state == 1 and now >= self.next_update_time:
            if self.update_car():
                ...
                self.speed = min(84.0, self.speed + 80)
    ...
    def draw(self, screen: Screen, now: float) -> None:
        ...
        for y, (left_col, right_col) in enumerate(self.track):
            ...
            # Green grass
            screen.draw.filled_rect(Rect(0, top, left, CELL_HEIGHT), "LimeGreen")
            screen.draw.filled_rect(Rect(right, top, COLS * CELL_WIDTH - right, CELL_HEIGHT), "HotPink")

            # Kerbs
            screen.draw.filled_rect(
                Rect(left, top, CELL_WIDTH, CELL_HEIGHT),
                ["cyan", "lemon chiffon"][(y + self.track_kerb_color) & 1]
            )
            screen.draw.filled_rect(
                Rect(right - CELL_WIDTH, top, CELL_WIDTH, CELL_HEIGHT),
                ["dark orchid", "goldenrod"][(y + self.track_kerb_color + 1) & 1]
            )
    ...
    def draw_car(self, surface: pygame.Surface, position: pygame.Rect, color: tuple = "DarkKhaki") -> None:
